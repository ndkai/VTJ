...
class TabBarWidget extends StatelessWidget implements PreferredSize {
  TabBarWidget({Key key, @required this.tag, @required this.tabs, @required this.initialSelectedId}) : super(key: key) {
    tabs[0] = Padding(padding: const EdgeInsetsDirectional.only(start: 15), child: tabs.elementAt(0));
    tabs[tabs.length - 1] = Padding(padding: const EdgeInsetsDirectional.only(end: 15), child: tabs[tabs.length - 1]);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(() {
      return RawChip(
        elevation: 0,
        label: Text(text),
        labelStyle: controller.isSelected(id) ? Get.textTheme.bodyText2.merge(TextStyle(color: Get.theme.primaryColor)) : Get.textTheme.bodyText2,
