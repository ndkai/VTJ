      ...
      child: Scaffold(
        body: ListView(
          ...
          children: [
            ...
            Obx(() {
              return UserAvatarVer(
                phone: controller.employeeInfo.value.data == null
                    ? ""
                    : controller.employeeInfo.value.data.phoneNumber,
                name: controller.employeeInfo.value.data == null
                    ? ""
                    : controller.employeeInfo.value.data.name,
                showDetail: true,
              );
            }),
            horiFunc("assets/icon/user_with_bg.png", "Thông tin cá nhân",
                ontap: () async {
              Provinces provinces = await Helper().getProvinceFormJson(context);
              if (controller.employeeInfo.value.data.phoneNumber == null) {
                Get.showSnackbar(
                    Ui.ErrorSnackBar(message: "Lấy thông tin thất bại"));
              } else {
                Get.toNamed(Routes.USER_INFOR, arguments: provinces);
              }
            }),
            horiFunc("assets/icon/lock_with_bg.png", "Cài đặt mật khẩu",
                ontap: () {
