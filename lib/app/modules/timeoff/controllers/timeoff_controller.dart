...
class TimeOffController extends GetxController {
  ...
  TextEditingController currentTimeOffTypeCl =
      TextEditingController(text: "Chọn loại nghỉ phép");
  BuildContext context;
  RxList<time_off.EmployeeTimeOff> employeesTimeOff =
      <time_off.EmployeeTimeOff>[].obs;
  ...
  Future<void> getEmployeeTimeOff(BuildContext context) async {
    try {
      ...
      employeesTimeOff.value = await _timeOffRepository.getEmployeeTimeOff(
          currentWeek.value.days.first.toIso8601String(),
          currentWeek.value.days.last.toIso8601String(),
          0,
          code: currentUser.value.userInfo.employee != null
  ...
  Future<void> postTimeOff() async {
    try {
      if (currentTimeOffType.value.id == null) {
        Get.showSnackbar(
            Ui.RemindSnackBar(message: "Chưa chọn loại nghỉ phép"));
        return;
      }
      ...
      showAppSuccesDialog(context,
          title: "Xác nhận tạo đơn nghỉ phép thành công",
          homepageTap: () {
  ...
  Future<void> adminPostTimeOff() async {
    try {
      if (employees.value.data.firstWhere((element) => element.isChoose) ==
          null) {
      ...
      var timeOffRequest = PostTimeOffRequest(
          employeeId:
              employees.value.data.firstWhere((element) => element.isChoose).id,
          fromDate: fromDate.value.toIso8601String(),
          toDate: toDate.value.toIso8601String(),
          timeOffTypeId: currentTimeOffType.value.id,
          note: "",
  ...
  Future<void> getEmployeesByName(String keyword) async {
    try {
      ...
      getEmployeeLoading.value = true;
      var e = await _timeOffRepository.getEmployees(
